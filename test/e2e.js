...
async function main(){
  ...
  console.log('— pool8 v2 (physics v2 + spin + shot clock)');
  ...
  /* shot-clock foul: wait for layla's turn (AI replies fast), then age the clock past 9s */
  for (let w = 0; w < 20; w++){
    const cur = (await api('GET', '/api/rooms/' + plRoom, null, laylaTok)).j.room;
    if (cur.turn === 1 && cur.aimable) break;
    await sleep(400);
  }
  {
    const Database = require('better-sqlite3');
  ...
  T('pool8: after foul the clock restarted (fresh turn timestamp)', after.turnStartedAt > Date.now() - 4000, { t: after.turn, ts: after.turnStartedAt });
