# VEXORA — Free 24/7 Deployment on Hugging Face Spaces (no credit card)
...
`https://{your-name}-vexora.hf.space`, and — with the persistence layer built
...
3. **New token** → name `vexora` → type **Write** → **Create** → copy it.
...
```
1. Creates **private dataset repo** `{you}/vexora-db` → permanent DB backups
   (your players' data stays private — the Space itself is public).
2. Creates the **public Docker Space** `{you}/vexora` and uploads the app.
3. Sets secrets: `HF_TOKEN`, `HF_REPO`, `DB_PATH`, `ADMIN_PASS`,
   `SELF_PING_URL` (keeps it awake), `HF_BACKUP_MIN=3`.
4. Build finishes in ~2–4 min → `https://{you}-vexora.hf.space` is live 24/7.
5. Agent verifies with `scripts/verify-permanent.js` (18 checks) and hands
   you the URL.

## After deployment

- **App:** `https://{you}-vexora.hf.space` (Arabic RTL, WebSockets, mobile).
- **Admin:** `https://{you}-vexora.hf.space/#/admin` — sign in at `#/auth`
  with `admin@vexora.gg` + the `ADMIN_PASS` you chose (default `admin123`).
- **DB backup:** Admin dashboard → **⬇ Download DB backup**
  (checkpointed SQLite, opens in any SQLite browser).
- **DB restore:** replace `backup.db.gz` in the private `vexora-db` repo, or
