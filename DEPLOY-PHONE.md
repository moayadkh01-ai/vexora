# NoirCue — Phone Deployment Guide (Railway, ~3 minutes)
...
| Production env | `NODE_ENV`, `DB_PATH=/data/noircue.db`, `COOKIE_SECURE`, `ADMIN_PASS` |
...
- **Login first** at `#/auth` with:
  - **Email/username:** `admin@noircue.gg`
...
- **Location:** `/data/noircue.db` on the persistent volume (set via `DB_PATH`).
...
- **Restore / inspect:** the file is a standard SQLite DB — open it with any
  ...
  `/data/noircue.db` (Railway dashboard → your service → Volumes).
