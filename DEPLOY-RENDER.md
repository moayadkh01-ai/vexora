# NoirCue — Render Free Path (no credit card)
...
3. **Generate new token (classic)** → note: `noircue` → expiration: 30 days →
...
```
GH_TOKEN=… RENDER_API_KEY=… HF_TOKEN=… HF_REPO=MoayadQ8/noircue-db \
  bash scripts/deploy-render.sh
```
1. Creates the public GitHub repo `yourname/noircue` and pushes the app
...
3. Sets environment: `ADMIN_PASS`, `HF_TOKEN`, `HF_REPO`, `SELF_PING_URL`,
   `COOKIE_SECURE`.
4. Build ~3-5 min → **https://noircue.onrender.com** (WebSockets supported).
...
> repository → noircue** (~6 taps) — everything after that is automated again.

## After go-live
- App: `https://noircue.onrender.com` · Admin: `/#/admin`
  (`admin@noircue.gg` + your `ADMIN_PASS`)
