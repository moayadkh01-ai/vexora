#!/bin/bash
# ============================================================
# VEXORA — Render deployment (run by the agent)
# Env: GH_TOKEN=<github PAT>  RENDER_API_KEY=<render key>
#      HF_TOKEN=<hf write token>  HF_REPO=<user>/vexora-db
...
curl -s -X POST https://api.github.com/user/repos -H "Authorization: token $GH_TOKEN" \
  -H 'Content-Type: application/json' -d '{"name":"vexora","private":false,"auto_init":true}' | head -c 120; echo

echo "▶ pushing files…"
cd "$DIR"
git init -q 2>/dev/null; git config user.email "deploy@vexora.gg"; git config user.name "VEXORA Deploy"
...
git commit -qm "VEXORA deploy" 2>/dev/null
git branch -M main 2>/dev/null
git push -qf "https://$GH_USER:$GH_TOKEN@github.com/$GH_USER/vexora.git" main 2>&1 | tail -2
echo "▶ repo: https://github.com/$GH_USER/vexora"
...
CREATE=$(curl -s -X POST https://api.render.com/v1/services -H "Authorization: Bearer $RENDER_API_KEY" \
  -H 'Content-Type: application/json' -d "{
    \"type\":\"web_service\",\"autoDeploy\":\"yes\",\"branch\":\"main\",\"name\":\"vexora\",
    \"repo\":\"https://github.com/$GH_USER/vexora\",
...
[ -n "$SID" ] && echo "SERVICE_ID=$SID" || { echo "⚠ API create blocked — connect the repo once from the Render dashboard (New → Web Service → vexora) then rerun this script for env+deploy."; exit 3; }

echo "▶ setting environment…"
for KV in "ADMIN_PASS=${ADMIN_PASS:-admin123}" "HF_TOKEN=${HF_TOKEN:-}" "HF_REPO=${HF_REPO:-}" "SELF_PING_URL=https://vexora.onrender.com" "COOKIE_SECURE=1"; do
...
echo "✅ deployed → https://vexora.onrender.com (build ~3-5 min)"
