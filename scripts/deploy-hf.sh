#!/bin/bash
# ============================================================
# VEXORA — deploy to Hugging Face Spaces (100% free, no card)
# Run by the agent with the user's HF write token:
#     HF_TOKEN=hf_xxx bash scripts/deploy-hf.sh
# Creates:  • public Docker Space  → https://{owner}-vexora.hf.space
...
SPACE="$OWNER/vexora"; DATASET="$OWNER/vexora-db"
...
curl -s -o /dev/null -w "" -X POST https://huggingface.co/api/dataset/create \
  ...
  -d "{\"name\":\"vexora-db\",\"private\":true}" || true

# 2) public Docker Space (create if missing)
curl -s -o /dev/null -X POST https://huggingface.co/api/spaces \
  -H "Authorization: Bearer $HF_TOKEN" -H 'Content-Type: application/json' \
  -d '{"name":"vexora","sdk":"docker","private":false,"hardware":"cpu-basic"}' || true
...
nd = '\n'.join([
    json.dumps({'key': 'header', 'value': {'summary': 'VEXORA deploy', 'repoType': 'space'}}),
...
for KV in "HF_TOKEN=$HF_TOKEN" "HF_REPO=$DATASET" "NODE_ENV=production" "DB_PATH=/app/data/vexora.db" "COOKIE_SECURE=1" "ADMIN_PASS=${ADMIN_PASS:-admin123}" "PAYMENTS_SIMULATE=1" "SELF_PING_URL=https://$OWNER-vexora.hf.space" "HF_BACKUP_MIN=3"; do
  K="${KV%%=*}"; V="${KV#*=}"
...
echo "✅ deployed → https://$OWNER-vexora.hf.space (build takes ~2-4 min)"
echo "   verify:  node scripts/verify-permanent.js https://$OWNER-vexora.hf.space"
